...
public class MenuDataBaseTest {
    ...
    @Test
    public void TestSaveCombo() throws SQLException {
        Combo combo = new Combo(-1,"Комбо-Обед", new BigDecimal("400"), 20, Singularity.SPICY);
        MenuDataBase menuDataBase = new MenuDataBase();
        Garnish potato = new Garnish(7,"Картошка жаренная", new BigDecimal("100"), 10, Singularity.NOSINGULARITY);
        HotDrink tea = new HotDrink(9,"Чай", new BigDecimal("20"), 3, Singularity.NOSINGULARITY, 90);
        Soup borsh = new Soup(13,"Борщ", new BigDecimal("350.0"), 30, Singularity.SOUR);
        combo.putToList(potato);
        combo.putToList(tea);
        combo.putToList(borsh);
        menuDataBase.saveCombo(combo);
        FoodDatabaseLocal foodDatabaseLocal = new FoodDatabaseLocal();
        List<Dish> list = foodDatabaseLocal.downloadBD();
        for (Dish dish : list){
            if (dish.getCategory() == Category.COMBO){
                Combo c = (Combo) dish;
                assertEquals(c.getName(), "Комбо-Обед");
                assertEquals(c.getList().get(0).getName(), combo.getList().get(0).getName());
                assertEquals(c.getList().get(1).getName(), combo.getList().get(1).getName());
                assertEquals(c.getList().get(2).getName(), combo.getList().get(2).getName());
            }
        }
    }
